...
class PostgresAdapter {
    ...
    async createUser(userData) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO users (email, password, first_name, last_name, roles) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING *`, [email, password, firstName, lastName, defaultRoles]);
    ...
    async createRole(roleData) {
        ...
        try {
            ...
            const result = await client.query(`INSERT INTO roles (name, description, permissions) 
         VALUES ($1, $2, $3) 
         RETURNING *`, [name, description, permissionsArray]);
    ...
    async assignRoleToUser(userId, roleId) {
        ...
        try {
            ...
            await client.query('UPDATE users SET roles = $1, updated_at = $2 WHERE id = $3', [updatedRoles, new Date(), numericUserId]);
            return true;
        }
        catch (error) {
    ...
    async getUserRoles(userId) {
        ...
        try {
            ...
            // Get roles
            const rolesResult = await client.query('SELECT * FROM roles WHERE name = ANY($1)', [roleNames]);
    ...
    async getUserByResetToken(token) {
        ...
        try {
         ...
         AND reset_password_expires > $2`, [token, new Date()]);
            if (result.rows.length === 0) {
                return null;
            }
            return this.mapUserFromDb(result.rows[0]);
